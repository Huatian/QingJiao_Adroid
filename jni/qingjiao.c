...
static jmethodID show_toast_method_id;
...
static void set_ui_message (const gchar *message, CustomData *data) {
  ...
  (*env)->CallVoidMethod (env, data->app, show_toast_method_id, jmessage);
...
static jboolean gst_native_class_init (JNIEnv* env, jclass klass) {
  ...
  show_toast_method_id = (*env)->GetMethodID (env, klass, "showToast", "(Ljava/lang/String;)V");
  ...
  if (!custom_data_field_id || !show_toast_method_id || !on_gstreamer_initialized_method_id ||
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  if ((*vm)->GetEnv(vm, (void**) &env, JNI_VERSION_1_4) != JNI_OK) {
    __android_log_print (ANDROID_LOG_ERROR, "native", "Could not retrieve JNIEnv");
    return 0;
  }
  jclass klass = (*env)->FindClass (env, "com/qingjiao/ui/GstPlayerActivity");
